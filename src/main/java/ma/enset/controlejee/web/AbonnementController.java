...
import ma.enset.controleJEE.services.AbonnementService;
...
public class AbonnementController{
    ...
    @GetMapping(path = "/user/abonnements")
    public String abonnements(Model model,
                          @RequestParam(name = "page", defaultValue = "0")int page,
                          @RequestParam(name = "size", defaultValue = "5")int size,
                          @RequestParam(name = "motCle", defaultValue = "")String motCle){
        Page<Abonnement> pageAbonnements = abonnementService.searchAbonnementByKeyword(motCle, page, size);
        model.addAttribute("ListeAbonnements", pageAbonnements.getContent());
